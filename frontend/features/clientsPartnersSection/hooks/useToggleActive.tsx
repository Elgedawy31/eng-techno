...
export function useToggleActive() {
  ...
  const mutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await clientsPartnersSectionService.toggleActive(id);
      if (isSuccessResponse(response)) {
        return response.data;
      }
      throw new Error(response.message || "Failed to toggle clients & partners section status");
    },
    onSuccess: () => {
      ...
      toast.success("Clients & partners section status updated successfully!");
    },
    onError: (error: unknown) => {
      const errorMessage = getErrorMessage(error);
      toast.error(errorMessage || "Failed to update clients & partners section status");
    },
  });
