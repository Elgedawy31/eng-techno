...
export function useClientsPartnersSection() {
  ...
  const {
    ...
    queryKey: ["clientsPartnersSection"],
    queryFn: async ({ signal }) => {
      ...
      const response = await clientsPartnersSectionService.getClientsPartnersSection(combinedSignal);
      
      if (isSuccessResponse(response)) {
        return response.data;
      }
      throw new Error(response.message || "Failed to fetch clients & partners section");
    },
  });
  ...
  return {
    clientsPartnersSection: data || null,
    isLoading,
    isError,
    error,
    refetch,
