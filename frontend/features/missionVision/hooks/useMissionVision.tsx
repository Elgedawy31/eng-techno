...
export function useMissionVision() {
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      const response = await missionVisionService.getMissionVision(combinedSignal);
      
      if (isSuccessResponse(response)) {
        return response.data;
      }
      throw new Error(response.message || "Failed to fetch mission & vision");
    },
  });
