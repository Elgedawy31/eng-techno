...
export function useAboutPageContent() {
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      const response = await aboutPageContentService.getAboutPageContent(combinedSignal);
      
      if (isSuccessResponse(response)) {
        return response.data;
      }
      throw new Error(response.message || "Failed to fetch about page content");
    },
  });
