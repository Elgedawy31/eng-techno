...
export function useMediaCentre() {
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      const response = await mediaCentreService.getMediaCentre(combinedSignal);
      
      if (isSuccessResponse(response)) {
        return response.data;
      }
      throw new Error(response.message || "Failed to fetch media centre");
    },
  });
